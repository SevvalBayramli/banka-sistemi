...
public class BankaIslemleri extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BankaIslemleri frame = new BankaIslemleri();
		...
		btn1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if("-".equals(bit_tarih.getText()) && "-".equals(bas_tarih.getText() ) )
					sql_sorgu="SELECT DISTINCT * FROM banka.islem where  (hedef_id=123 or kaynak_id=123) ;";
				else if("-".equals(bas_tarih.getText()) && !("-".equals(bit_tarih.getText()))) {
					
					sql_sorgu="SELECT DISTINCT * FROM banka.islem where  (hedef_id=123 or kaynak_id=123) and tarih <= '"+bit_tarih.getText()+"';";
				}else if("-".equals(bit_tarih.getText()) && !("-".equals(bas_tarih.getText()))){
					
					sql_sorgu="SELECT DISTINCT * FROM banka.islem where  (hedef_id=123 or kaynak_id=123) and tarih >= '"+bas_tarih.getText()+"';";
				}else {
					
					sql_sorgu="SELECT DISTINCT * FROM banka.islem where  (hedef_id=123 or kaynak_id=123) and tarih between '"+bas_tarih.getText()+"' and '"
							+bit_tarih.getText()+"';";
				}
