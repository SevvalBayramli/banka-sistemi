...
public class ParaCekme extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ParaCekme frame = new ParaCekme(musteri);
	 ...
	 * @param musteri
	 */
	public ParaCekme(Musteri musteri) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (bakiye > 0.0) {
					String sql_sorgu = "UPDATE hesap SET bakiye=" + (bakiye - Integer.parseInt(tutar))
							+ " where hesap_id=" + hesapId;
					veritabani_baglanti.update(sql_sorgu);
					String sql="INSERT INTO islem(hedef_id,kaynak_id,islem_adi,tarih,kaynak_bakiye,hedef_bakiye,tutar) VALUES ("+hesapId + ", " +hesapId +",'paracekme' ,'"+date.toString()+"',"+bakiye+","+bakiye+","+tutar +")";
